 ...
 * @author 9ins
 */
public class ResourceProviderConfig <T> extends Metadata <T> {

    private Map<String, String> watchRoots = super.<Map<String, String>>getValue("resource-provider.watch-roots");
    private List<String> watchKind = super.<List<String>>getValue("resource-provider.watch-kind");
    private List<String> accessFilters = super.<List<String>>getValue("resource-provider.access-filters");;
    private List<String> inMemoryFilters = super.<List<String>>getValue("resource-provider.in-memory-filters");;
    private long totalMemorySizeLimit = ResourceUtils.fromString(super. getValue("resource-provider.total-memory-size-limit"));
    private int splitUnitSize = (int) ResourceUtils.fromString(super. getValue("resource-provider.split-unit-size"));
    private int fileSizeLimit = (int) ResourceUtils.fromString(super. getValue("resource-provider.file-size-limit"));
    private int fileReadBufferSize = (int) ResourceUtils.fromString(super. getValue("resource-provider.file-read-buffer-size"));
    private int fileWriteBufferSize = (int) ResourceUtils.fromString(super. getValue("resource-provider.file-write-buffer-size"));

    /**
     * Constructor
     * @param configMap
     */
    public ResourceProviderConfig(T configMap) {
        super(configMap);
    }

    public Map<String, String> getWatchRoots() {
        return watchRoots;
    }

    public void setWatchRoots(Map<String, String> watchRoots) {
