...
import org.chaostocosmos.leap.http.service.model.StreamingModel;
...
public abstract class AbstractStreamingService extends AbstractService implements StreamingModel {
    ...
    public AbstractStreamingService(MIME_TYPE mimeType) {
        if(mimeType != MIME_TYPE.VIDEO_MP4 
           ...
           throw new HTTPException(RES_CODE.RES415, Context.getMessages().<String>getErrorMsg(22, "Specified media type is not supported: "+mimeType.mimeType()));
        }
    }
    ...
    public void GET(final Request request, final Response response) throws Exception {
        ...
        if(reqFile == null || reqFile.equals("")) {
            throw new HTTPException(RES_CODE.RES412, "Parameter not found(file). Streaming request must have field of file.");
        }
        Path resourcePath = super.serviceManager.getHost().getStatic().resolve(reqFile);
        if(!resourcePath.toFile().exists()) {
            throw new HTTPException(RES_CODE.RES404, "Specified resource not found: "+resourcePath.toAbsolutePath().toString().replace("\\", "/"));
        }
        String range = request.getReqHeader().get("Range");
        if(range == null || range.equals("")) {
            throw new HTTPException(RES_CODE.RES412, "Header field not found(Range). Streaming request header must have field of Range");
        }
